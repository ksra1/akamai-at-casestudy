    ...
    <SlideLayout id="security-analysis" pageNumber={7}>
      <div className="space-y-5">
        <div className="text-center space-y-2">
          <p className="text-accent font-semibold tracking-[0.2em] uppercase text-sm">Section C — Security Analysis</p>
          <h2 className="font-display text-4xl md:text-5xl font-bold text-foreground">Security & Governance Analysis</h2>
        </div>
        ...
        <div className="glass-card rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-display font-bold text-foreground text-sm">Scale & Governance Challenges Addressed in This Section</h3>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            {scaleGovMapping.map(item => (
              <div key={item.challenge} className="flex items-start gap-3 bg-akamai-green/5 border border-akamai-green/15 rounded-xl p-4">
                ...
                <div>
                  <p className="text-xs font-bold text-foreground/80">{item.challenge}</p>
                  <p className="text-[11px] text-foreground/40 mt-1">{item.solution}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {impacts.map(imp => (
            <div key={imp.area} className="glass-card rounded-xl p-4">
              ...
              <ul className="space-y-2">
                {imp.items.map(item => (
                  <li key={item} className="flex items-start gap-2 text-xs text-foreground/40">
            ...
            {[
              ...
              <div key={i} className="flex items-center gap-2">
                <div className="flex-1 bg-destructive/8 border border-destructive/15 rounded-lg p-2.5 text-center">
                  <p className="text-[11px] font-bold text-foreground/70">{chain.steps[0]}</p>
                  <p className="text-[9px] text-foreground/30">{chain.steps[1]}</p>
                </div>
                ...
                <div className="flex-1 bg-accent/8 border border-accent/15 rounded-lg p-2.5 text-center">
                  <p className="text-[11px] font-bold text-foreground/70">{chain.steps[2]}</p>
                  <p className="text-[9px] text-foreground/30">{chain.steps[3]}</p>
                </div>
                <ArrowRight size={14} className="text-foreground/20 shrink-0" />
                <div className="flex-1 bg-primary/8 border border-primary/15 rounded-lg p-2.5 text-center">
                  <p className="text-[11px] font-bold text-foreground/70">{chain.steps[4]}</p>
                  <p className="text-[9px] text-foreground/30">{chain.steps[5]}</p>
                </div>
                <div className="shrink-0 w-20">
                  <div className="bg-accent/10 border border-accent/20 rounded-lg px-2 py-1 text-center">
        ...
        {/* Key insight */}
        <div className="bg-primary/8 border border-primary/15 rounded-xl p-4 flex items-start gap-3">
